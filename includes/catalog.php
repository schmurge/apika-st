...
<?php elseif (isset($_GET['group']) && !empty($_GET['group'])) : ?>
	<?php if (!empty($_GROUPS) && is_array($_GROUPS)) : ?>
		<?php foreach($_GROUPS as $group) : ?>
			<?php if ($group['id'] == $_GET['group']) : ?>
				<?php if (isset($group['products']) && is_array($group['products']) && !empty($group['products'])) : ?>
					<div class="row">
						<div class="col-md-12">
							<?php foreach ($group['products'] as $product) : ?>
								<div class="col-md-4">
									<a href="/catalog.php?product=<?php echo $product['productId']; ?>">
										<div class="grey-box">
											<?php if (!empty($product['thumbName']) && file_exists(PRODUCT_IMAGES_PATH.$product['productId'].'/'.PRODUCT_THUMBS_FOLDER_NAME.'/'.$product['thumbName'])) : ?>
												<img src="<?php echo PRODUCT_IMAGES_URL.$product['productId'].'/'.PRODUCT_THUMBS_FOLDER_NAME.'/'.$product['thumbName']; ?>">
											<?php else: ?>
												<img src="http://dummyimage.com/232x201/<?php echo $colors[$product['productId']]; ?>/000.gif&text=<?php echo $product['label']; ?>">
											<?php endif; ?>
